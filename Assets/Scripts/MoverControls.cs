...
public class MoverControls : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (walkPreviousMoveData.moverObjectsParameters.Length == moverObjects.Length)
        {
            ...
            nextMoveSpeed = walkNextMoveData.moveSpeed;
    ...
    void Update()
    {
        ...
        // ** might not need these next two lines
        transitionTrigger = walkAnimsManager.transitionTrue;
        transitionDuration = walkPreviousMoveData.transitionDuration;
        ...
        if(!movingRootPosition)
            moverTime += Time.deltaTime * previousMoveSpeed; // use this if movement is NOT related to character movement in world space
        if (movingRootPosition) 
        {
        ...
        bool isTransitioning = (transitionValue > 0) ? true : false;
 ...
 /*   private IEnumerator TransitionTimer() 
    ...
    {
        float walkToJog = playerController.movementInput.magnitude / 1f;
        if (walkToJog < startWalkingThreshold) 
        {
            transitionValue = Mathf.InverseLerp(0, startWalkingThreshold, walkToJog);
        }
        else if (walkToJog >= startWalkingThreshold)
        {
